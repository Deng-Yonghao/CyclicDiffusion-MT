...
def _compute_sidechain_chi(residue, aa_name):
    ...
    Follows standard chi definitions. Returns a (4,) tensor; unused chi
    slots are zero.

    chi1: N-CA-CB-CG   (atoms 0,1,4,5)
    chi2: CA-CB-CG-CD  (atoms 1,4,5,6)
    chi3: CB-CG-CD-CE  (atoms 4,5,6,7)
    chi4: CG-CD-CE-NZ  (atoms 5,6,7,8)
    """
    ...
    chi_atom_indices = [
        ...
        (1, 4, 5, 6),  # chi2: CA-CB-CG-CD
        (4, 5, 6, 7),  # chi3: CB-CG-CD-CE
        (5, 6, 7, 8),  # chi4: CG-CD-CE-NZ
    ]

    for k in range(min(n_chi, 4)):
        indices = chi_atom_indices[k]
        if all(idx < len(atom_coords) for idx in indices):
            chi[k] = _dihedral(
                atom_coords[indices[0]],
